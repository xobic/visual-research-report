...
Each fact appears as one focusable target per local visual context. Clicking it opens the evidence drawer. Decorative marks stay `aria-hidden`. Chart wrappers expose `data-chart-type`; stack shares and line scroll behavior expose QA attributes. Never encode kind or sign by color alone.
